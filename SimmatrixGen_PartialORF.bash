...
tblastn -num_threads 8 -query "$DBPATH"/"$1".fasta -outfmt 6 -db "$DBPATH"/"$DBNAME" > "$1"_"$DBNAME".tblastn_tab
...
#	| tr " " "\t" |  awk -v SS=1 '{SS = ($9 - ( ( $7 - 0 ) * 3))  ;print "blastdbcmd -db '"$DBPATH"'/'"$DBNAME"' -entry "$2" -range "SS "-"$10 + 30 }' | bash \
#	| awk '{if ($1 ~ ">") {print "#" $0 "#"} else {print $0} }' | tr -d "\n" | tr "#:" "\n " > "$1"_"$DBNAME"_nucl.fasta

#RBD and nstp
awk '{print $0 "\t" substr($2,2) }' "$1"_"$DBNAME".tblastn_tab | awk '{ gsub("_","\t",$13) ;print $0}' | sort -h -k12,12gr -k4,4gr | sort -u -h -k13,13g  \
	| tr " " "\t" |  awk -v SS=1 '{SS = ($9 - ( ( $7 - 1 ) * 3))  ;print "blastdbcmd -db '"$DBPATH"'/'"$DBNAME"' -entry "$2" -range "SS"-"$10  }' | bash \
	| awk '{if ($1 ~ ">") {print "#" $0 "#"} else {print $0} }' | tr -d "\n" | tr "#:" "\n " > "$1"_"$DBNAME"_nucl.fasta
...
mafft --auto --maxiterate 1000 "$1"_"$DBNAME"_AA.fasta  2> mafftstderr \
